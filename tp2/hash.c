...
struct hash{
	array_hash_t* tabla;					//	Arreglo conformado por nodos de hash.
...
array_hash_t* crear_tabla(size_t tamanio){
	array_hash_t* tabla = malloc(tamanio * sizeof(array_hash_t));
	if (!tabla){
		return NULL;
	}
...
bool redimension(hash_t* hash, size_t nuevo_tam){
	...
	array_hash_t* ant_tabla = hash -> tabla; 
